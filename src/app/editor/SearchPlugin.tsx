...
import { $createRangeSelection, $getRoot, $isTextNode, $setSelection, TextNode, LexicalNode } from 'lexical';
...
export function SearchPlugin() {
  ...
  const pendingMatchOffsetRef = useRef<number | null>(null);
  const lastContentRef = useRef<string>('');

  useEffect(() => {
    const handleExternalSearch = (event: Event) => {
      const customEvent = event as CustomEvent<{ query?: string; matchIndex?: number }>;
  ...
  useEffect(() => {
    ...
    const unregister = editor.registerUpdateListener(({ editorState }) => {
      ...
      if (textContent === lastContentRef.current) return;
