...
- The header shows a **version badge**: the published `@liminis/editor`
  version this build represents. The Pages build sets it from the triggering
  release's tag, so the live site always names the version it was actually
  deployed from — which is not necessarily the version in `package.json` on
  `main`, since that can already have moved on to the next one. A local build
  has no release behind it and reads "unpublished (local build)".
